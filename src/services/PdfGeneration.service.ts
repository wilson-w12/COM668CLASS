...
export class PdfGenerationService {
    ...
    public drawTableHeader(doc: jsPDF, headers: string[], columnWidths: number[], startY: number): void {
        ...
        headers.forEach((header, i) => {
            ...
            doc.rect(currentX, startY - 6, columnWidths[i], 8, "F"); // Fill rectangle
            doc.text(header, currentX, startY - 2); // Left-aligned text
        ...
        doc.line(10, startY + 2, 10 + totalWidth, startY + 2);
    }

    // Draw table row (left-aligned text)
    public drawTableRow(doc: jsPDF, rowData: string[], columnWidths: number[], startY: number): void {
        rowData = rowData.map(value => (value != null && value !== '') ? value.toString() : 'N/A');

        if (isNaN(startY)) {
            console.error('Invalid Y coordinate:', startY);
            startY = 10;
        }
        ...
        rowData.forEach((cellData, i) => {
            if (cellData && !isNaN(currentX) && !isNaN(startY)) {
                doc.text(cellData, currentX + 2, startY + 5); // Left-aligned text
            }
            currentX += columnWidths[i];
        });
    }
