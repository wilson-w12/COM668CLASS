...
export class AddStudentComponent implements OnInit {
  ...
  getClassesForSubject(subject: string): void {
    this.teacherService.getClasses({ subject }).subscribe(
      (response) => {
        ...
        this.autoSelectTeachers(); // Auto-select after data is loaded
      },
      (error) => {
        console.error('Error fetching classes for subject:', subject, error);
      }
  ...
  // Clear the value if it's not valid (helper function)
  clearIfNotValid(
    control: FormControl,
    validOptions: { class_id: string, name: string, year: string, set: string }[] | string[]
  ): void {
    if (validOptions.length === 0) return; // Guard clause for empty array

    // Handle string[] validation (e.g., genders, years, sets)
    if (typeof validOptions[0] === 'string') {
      if (!(validOptions as string[]).includes(control.value)) {
        control.setValue('');
      }
    } else {
      // Handle { class_id, name, year, set }[] validation
